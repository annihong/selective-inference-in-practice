...
max_t_mul_boot_k <- function(xx, yy, k, 
  ...
  boot_sample_C <- tmax::max_t_mul_boot_by_k(xx, yy, EE, sandwich, Nboot, varlist, individual)
...
# return maxt, beta, se, by model size k
max_t_k <- function(xx, yy, k, 
                    sandwich=TRUE, return_sample=TRUE, force_in=NULL, force_out=NULL,
                    intercept = T, individual = NULL) {
  if(is.data.frame(xx)) xx <- as.matrix(xx)
  if(intercept) {
    xx <- cbind(1, xx); force_in = c(1, force_in+1); k <- k + 1
    if(!is.null(individual)) individual <- individual + 1
  ...
  if (!k) {
    ...
    varlist <- gen_submodel_indices_k(k, p, force_in = force_in, force_out = force_out)
  }
  num_submodels <- nrow(varlist)
  ...
  boot_sample_C <- max_t_by_k(xx, yy, sandwich, varlist, individual)
  
  boot_rank <- rank(boot_sample_C$max_t)
...
# return maxt, beta, se, by model M
max_t_M <- function(xx, yy, M, 
                    sandwich=TRUE, return_sample=TRUE,
                    intercept = T, individual = NULL) {
  if(is.data.frame(xx)) xx <- as.matrix(xx)
  if(intercept) {
    xx <- cbind(1, xx); M <- cbind(1, M + 1)
    if(!is.null(individual)) individual <- individual + 1
  }
  ...
  # get varlist
  num_submodels <- nrow(M)
  
  if(return_sample) {
    zz <- rep(0, p)
    all_submodels <- apply(M, 1, function(k) {zz[k]=1;paste0(zz, collapse = "")})
  }
  
  boot_sample_C <- max_t_by_k(xx, yy, sandwich, M, individual)
  ...
  if(!is.null(individual)) {
    ...
    ret <- list(X = xx, Y = yy,
                beta = boot_sample_C$beta, se = boot_sample_C$se,
                maxt = boot_sample_C$max_t, 
                maxtRank = boot_rank,
                maxt1 = boot_sample_C$max_t1, 
                maxtRank1 = boot_rank1,
                AllModelNames = all_submodels)
  } else {
    ret <- list(X = xx, Y = yy, 
                beta = boot_sample_C$beta, se = boot_sample_C$se,
