...
min_p_boston <- function(boston_null){
  ...
  nox_trans <- (boston_null$nox)^selected_p
...
train_prop <- c()
test_prop <- c()
sample_size_seq <- seq(from=100,to=10000, by=2000)
...
#par(mfcol=c(2,1))
plot(sample_size_seq, test_prop, type = "l", col="blue", ylim = c(0.3,0.7), ylab = "P(at least 1 sig term under global null) ", main = "FDR for post transformation inference, test vs train (Boston)")
...
legend("topleft", legend=c("train lm", "test lm"),col=c("red", "blue"), lty=1:1, cex=1)
```
