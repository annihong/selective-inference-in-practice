...
select_power <- function(data){
  powers <- c(1,0)
<<<<<<< HEAD
  fit1 <- lm(data$y ~ -1 + data$X[,1] + boxcox_transform((data$X[,2]),1))
  fit2 <- lm(data$y ~ -1 + data$X[,1] + boxcox_transform((data$X[,2]),0))
=======
  fit1 <- lm(data$y ~ data$X[,1] + data$X[,2])
  fit2 <- lm(data$y ~ data$X[,1] + boxcox_transform((data$X[,2]),0))
>>>>>>> b4ccba1d7aeb2dcbd4a16611a6fdcdf72656ce15
  models <- list(fit1,fit2)
  rss <- sapply(models, deviance)
  idx <- which.min(rss)
  power <- powers[idx]
<<<<<<< HEAD
  alpha_hat <- models[[idx]]$coefficients[1]
  beta_hat <- models[[idx]]$coefficients[2]
=======
...
p_seq <- seq(0.367, 1, 0.2)
plots <- list()
i=1
for (p in p_seq) {
  raw <- data.frame(var_consistency(power_f = function(x) p, n = n, B = B))
  print(mean(raw$power))
  print(mean(raw$a_hat))
  print(mean(raw$b_hat))
raw$a_hat <- (raw$a_hat) * sqrt(n)
 plot_a <- raw %>% ggplot(aes(x=a_hat)) +
     geom_histogram(aes(y=..density..), bins=50, alpha=0.8) + 
  ...
  theme(plot.title = element_text(hjust = 0.5), text = element_text(size=8)) + 
   ylab('') + xlab("") + ggtitle(paste('lambda^0 =', p))
...
for (p in p_seq) {
  raw <- data.frame(var_consistency(power_f = function(x) p, n = n, B = B))
raw$b_hat <- (raw$b_hat - 0.5) * sqrt(n)
 plot_a <- raw %>% ggplot(aes(x=b_hat)) +
     geom_histogram(aes(y=..density..), bins=50, alpha=0.8) + 
    scale_fill_manual(values=c("#69b3a2", "#404080")) +
    geom_density() +
    #labs(fill="", title = paste("distribution of alpha_hat,", p)) +
